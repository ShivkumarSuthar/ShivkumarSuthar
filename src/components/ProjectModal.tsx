...
export const ProjectModal: React.FC<ProjectModalProps> = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      >
        {/* Modal Header */}
        <div className="p-6 border-b border-[var(--border-subtle)] bg-[var(--bg-header)] text-white flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <span className="text-xs font-bold uppercase tracking-wider text-[#f8fafc] bg-white/20 px-2 py-0.5 rounded">
                {project.category}
              </span>
              <span className="text-xs font-semibold px-2 py-0.5 rounded bg-[#2563eb] text-white">
                {project.status}
              </span>
            </div>
          ...
          <button
            ...
            className="p-2 rounded-lg text-white hover:bg-white/15 transition-colors shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="flex items-center gap-2">
            {project.liveUrl && (
              <a
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold text-white bg-[#2563eb] hover:bg-[#1d4ed8] transition-colors shadow-xs"
              >
